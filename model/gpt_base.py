...
try:
    from gpt_shared import build_rope_cos_sin, merge_attention_heads, project_qkv, rotate_half
...
class RotaryEmbedding(nn.Module):
    ...
    def _build_cos_sin(self, seq_len, device, dtype):
        ...
        return build_rope_cos_sin(self.inv_freq, positions, dtype)
...
class GPTBase(nn.Module):
    """Shared GPT architecture and initialization for inference variants.

    Owns embeddings, transformer blocks, final normalization, and LM head.
    Preserves weight tying and GPT-2 initialization conventions.
    Leaves the decode algorithm to concrete subclasses.
    """
    ...
    def embed_inputs(self, x, start_pos=0):
        # Embed token ids and add learned positions when configured.
        seq_length = x.size(1)
        x = self.tok(x)
        if self.pos_embed_type == "learned":
            positions = torch.arange(start_pos, start_pos + seq_length, device=x.device).unsqueeze(0)
